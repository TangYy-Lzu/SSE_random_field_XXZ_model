...
# 清空或创建测量结果文件
open("SSE-Heisenberg_1D/res_1D.dat", "w") do f
    # 创建并清空文件
end

# 清空或创建测量结果文件
open("SSE-Heisenberg_1D/cut.text", "w") do f
    # 创建并清空文件
end

function mainprogram(lx::Int64, beta::Float64, nbins::Int64, msteps::Int64, isteps::Int64)
    # lx: 链长, beta: 逆温, nbins: bin 数量, msteps: 测量步数, isteps: 平衡步数
    nn::Int64 = lx         # 1D 链，总自旋数即链长
    nb::Int64 = nn         # 1D 链中键数为 nn
    ...
    bsites = SSELattice.makelattice(nn, lx, bsites)  # 调用 SSELattice 模块中的函数
    ...
    for j = 1:nbins
        ...
        for i = 1:msteps
            nh = SSEUpdate.diagonalupdate(nh, mm, opstring, spin, spin_mm, bsites, nb, beta, Δ, J, h, W,)
            SSEUpdate.loopupdate(mm, nn, opstring, bsites, vertexlist, spin, spin_mm, Δ, J, h, W)
            (enrg1, enrg2, amag, amag2, MRC) = SSEMeasure.measureobservables(mm, nh, nn, lx, spin, bsites, opstring, enrg1, enrg2, amag, amag2, MRC)
        end
...
if isfile("SSE-Heisenberg_1D/read_1D.in")
    f = open("SSE-Heisenberg_1D/read_1D.in", "r")
...
@time mainprogram(lx, beta, nbins, msteps, isteps)
